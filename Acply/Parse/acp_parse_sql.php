...
// 解析条件表达
    private static function parsePrepareCondition(array $ary, array & $param)
    {
        $len = count($ary);
...
// 接受传来需要解析成SQL的信息
    public static function receive($t, array $tb, array $ree)
    {
        // 初始化各种值
        self::$field = '*';
    ...
    public static function makePrepare()
    {
        $param = array(); // 需要绑定的值
        $str = ''; // preapre需要的值
    ...
    public static function bindParam(PDOStatement $p, array $ary)
    {
        foreach ($ary as $key => $value) {
