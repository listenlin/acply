<?php
	...
	class Acp_util extends Acp_base {
		 ...
		 */
		static public function createValidateCode($fs = 15, $len = 4, $uimg = false) {
		 ...
		 */
		static public function getFileSQL($dir) {
			if (file_exists($dir)) {
				$preg = array(
					'/\/\*.*\*\//Us', // 匹配多行/**/式注释
					'/^\s*/m', // 匹配从一行开始的空白字符
		 ...
		 * @return string
		 */
		static public function getDbTime() {
			return date('YmdHis');
		}
		/**
		 * 给用户发送404未找到错误
		 ...
		 * @return string
		 */
		static public function urlToLocal($dir,$prefix = NULL){
			if(!$dir) return '';
			if (!$prefix) {
		 ...
		 * @return string
		 */
		static public function localToSslUrl($dir, $prefix = NULL){
			if(!$dir) return '';
			if (is_array($dir)) $dir = empty($dir['dir']) ? '' : $dir['dir'];
			if (!$prefix) {
		 ...
		 * @return string
		 */
		static public function sslUrlToLocal($dir,$prefix = NULL){
			if(!$dir) return '';
			if (!$prefix) {
				$prefix = 'https://' . $_SERVER['SERVER_NAME'];
			} elseif ($prefix instanceof Smarty_Internal_Template) {
         ...
         * @return array RGB数组
         */
        static public function hColor2RGB($hexColor) {
